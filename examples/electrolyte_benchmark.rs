// Benchmark comparing ripopt vs ipopt on electrolyte thermodynamics problems.
//
// Run with: cargo run --release --features ipopt-native --example electrolyte_benchmark
...
// =========================================================================
// Ipopt C API FFI (copied from examples/benchmark_solvers.rs)
// =========================================================================
...
mod ipopt_ffi {
    ...
    extern "C" fn eval_jac_g_cb(n: i32, x: *const f64, _: bool, _m: i32, _nj: i32, ir: *mut i32, jc: *mut i32, vals: *mut f64, ud: *mut c_void) -> bool {
        unsafe {
            let w = &*(ud as *const IpoptWrapper);
            if vals.is_null() {
    ...
    extern "C" fn eval_h_cb(n: i32, x: *const f64, _: bool, obj_factor: f64, _m: i32, lambda: *const f64, _: bool, _nh: i32, ir: *mut i32, jc: *mut i32, vals: *mut f64, ud: *mut c_void) -> bool {
        unsafe {
            let w = &*(ud as *const IpoptWrapper);
            if vals.is_null() {
    ...
    extern "C" fn intermediate_cb(_: i32, iter: i32, _: f64, _: f64, _: f64, _: f64, _: f64, _: f64, _: f64, _: f64, _: i32, ud: *mut c_void) -> bool {
    ...
    pub fn ipopt_status_str(s: i32) -> String {
        match s { 0 => "Optimal".into(), 1 => "Acceptable".into(), 2 => "Infeasible".into(), -1 => "MaxIter".into(), -2 => "RestorationFailed".into(), other => format!("Status({})", other) }
    }
    ...
    pub fn solve_ipopt(problem: &dyn NlpProblem, tol: f64, max_iter: i32) -> super::SolveResult {
        ...
        unsafe {
            ...
            let mut g = vec![0.0; m.max(1)]; let mut obj = 0.0;
            let mut mg = vec![0.0; m.max(1)]; let mut ml = vec![0.0; n]; let mut mu = vec![0.0; n];
...
fn main() {
    ...
    println!();
    println!("Electrolyte Thermodynamics Benchmark: ripopt vs ipopt");
    println!("====================================================");
    println!();
    ...
    if have_ipopt {
        header += &format!(" | {:>11} {:>5} {:>8}", "ipopt obj", "iter", "time(s)");
    }
    println!("{}", header);
    ...
    println!("--- Speciation / Chemical Equilibrium ---");
    bench!("Water autoionization", WaterAutoionization);
    bench!("CO2-water speciation", Co2WaterSpeciation);
    bench!("NaCl speciation", NaClSpeciation);
    bench!("CaCl2+NaCl mixed", CaCl2NaClMixed);
    bench!("Phosphoric acid", PhosphoricAcid);

    println!("--- Phase Equilibrium ---");
    bench!("HCl mean activity", HclMeanActivity);
    bench!("NaCl solubility", NaClSolubility);
    bench!("BuOH-water LLE", ButanolWaterLle);
    bench!("Saturated brine", SaturatedBrine);

    println!("--- Parameter Fitting ---");
    bench!("Pitzer NaCl fit", PitzerNaClFit);
    bench!("Multi-salt DH fit", MultiSaltDhFit);
    bench!("eNRTL T-dep fit", EnrtlTempFit);
